<template>
  	<div class="paddingTop search_page">
        ...
        <form class="search_form">
            <input type="search" name="search" placeholder="请输入美食名称" class="search_input" v-model="searchValue" @input="checkInput">
            <input type="submit" name="submit" class="search_submit" @click.prevent="searchTarget('')">
        </form>

        <section v-if="restaurantList.code == '200'">
            <ul class="list_container">
                <router-link :to="{path:'/shop', query:{id:item.id}}" tag="li" v-for="item in restaurantList.data" :key="item.id" class="list_li">
                    <section class="item_left">
                        <img :src="item.thumb" class="restaurant_img">
                    </section>
